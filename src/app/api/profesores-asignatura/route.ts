...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        '(instructor LIKE ? OR asignatura LIKE ? OR catalogo LIKE ? OR cedula LIKE ?)'
      );
      const term = `%${search}%`;
    ...
    const countRow = db
      .prepare(
        `SELECT COUNT(*) as total FROM profesores_asignatura ${whereClause}`
      )
      .get(...params) as { total: number };

    const rows = db
      .prepare(
        `SELECT * FROM profesores_asignatura ${whereClause} ORDER BY id DESC LIMIT ? OFFSET ?`
      )
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = db
      .prepare(
        `INSERT INTO profesores_asignatura (${columns.join(', ')}) VALUES (${placeholders})`
      )
      .run(...values);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const result = db
      .prepare(`UPDATE profesores_asignatura SET ${setClause} WHERE id = ?`)
      .run(...values, id);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const result = db
      .prepare('DELETE FROM profesores_asignatura WHERE id = ?')
      .run(id);
