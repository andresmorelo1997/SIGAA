...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        '(programa_academico LIKE ? OR asignatura LIKE ? OR catalogo LIKE ?)'
      );
      const term = `%${search}%`;
    ...
    const countRow = db
      .prepare(`SELECT COUNT(*) as total FROM plan_estudios ${whereClause}`)
      .get(...params) as { total: number };

    const rows = db
      .prepare(
        `SELECT * FROM plan_estudios ${whereClause} ORDER BY id DESC LIMIT ? OFFSET ?`
      )
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = db
      .prepare(
        `INSERT INTO plan_estudios (${columns.join(', ')}) VALUES (${placeholders})`
      )
      .run(...values);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const result = db
      .prepare(`UPDATE plan_estudios SET ${setClause} WHERE id = ?`)
      .run(...values, id);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const result = db
      .prepare('DELETE FROM plan_estudios WHERE id = ?')
      .run(id);
